...
const AdminStudents = () => {
    ...
    useEffect(() => {
        api.adminListStudents().then(setStudents).catch(() => { });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSuccess(`Student ${form.full_name} created!`);
            setForm(emptyForm);
            const updated = await api.adminListStudents();
            setStudents(updated);
        } catch (err) {
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-2xl font-bold text-[#0f2a5f]">Add Student</h1>
            ...
            <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <Input label="Batch Year" name="batch_year" type="number" value={form.batch_year} onChange={handleChange} required />
                    <Input label="Current Semester" name="current_semester" type="number" value={form.current_semester} onChange={handleChange} required />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100">
                    <h2 className="font-semibold text-[#0f2a5f]">Existing Students ({students.length})</h2>
                </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {students.map((s) => (
                            <tr key={s.id} className="hover:bg-gray-50">
                                <td className="px-6 py-3 font-medium text-gray-800">{s.full_name}</td>
                                <td className="px-6 py-3 text-gray-600">{s.email}</td>
                                <td className="px-6 py-3 text-gray-600">{s.student?.roll_number || '—'}</td>
                                <td className="px-6 py-3 text-gray-600">{s.student?.batch_year || '—'}</td>
                            </tr>
                        ))}
                        {students.length === 0 && (
